...
class AnalyticsResponse {
  ...
  extractRows() {
    ...
    if (headersWithOptionSet.length) {
      rows = rows.slice();
      const optionCodeIdMap = this.optionCodeIdMap(); // replace option code with option uid
  ...
  extractMetadata() {
    ...
    this.headers.filter(header => !DEFAULT_COLLECT_IGNORE_HEADERS.includes(header.name)).forEach(header => {
      ...
      if (header.isPrefix) {
        ...
        dimensions[header.name] = ids.map(id => getPrefixedId(id, header.name)); // create items
  ...
  getSortedUniqueRowIdStringsByHeader(header) {
    ...
    const rowIds = Array.from( // unique values
    new Set(this.rows.map(responseRow => parseByType(responseRow[header.index]))) // remove empty values
    ).filter(id => id !== '');
  ...
  optionCodeIdMap() {
    ...
    this.headers.filter(header => typeof header.optionSet === 'string').forEach(header => {
      ...
      map[header.name] = optionIds.map(id => ({
        [items[id].code]: id
