...
describe('AnalyticsRequest', () => {
  ...
  describe('constructor', () => {
    it('should not be allowed to be called without new', () => {
      expect(() => (0, _AnalyticsRequest.default)()).toThrowErrorMatchingSnapshot();
    });
  ...
  describe('properties', () => {
    ...
    describe('.fromVisualization()', () => {
      const visualization = _fixtures.default.get('/chartAllFields');
      it('should add dimensions from the visualization', () => {
        request = request.fromVisualization(visualization);
        expect(request.dimensions).toEqual([{
          dimension: 'dx',
      ...
      it('should add filters from the visualization', () => {
        request = request.fromVisualization(visualization);
        expect(request.filters).toEqual([{
          dimension: 'pe',
      ...
      it('should convert filters into dimensions when passing the optional flag', () => {
        request = request.fromVisualization(visualization, true);
        expect(request.dimensions).toEqual([{
          dimension: 'dx',
          items: ['Uvn6LCg7dVU', 'sB79w2hiLp8']
        }, {
    ...
    describe('with value parameter', () => {
      ...
      ['approvalLevel', 'asc',
      // XXX
      'bbox', 'clusterSize', 'columns',
      // XXX
      'desc',
      // XXX
      'endDate', 'inputIdScheme', 'measureCriteria', 'outputIdScheme', 'preAggregationMeasureCriteria', 'relativePeriodDate', 'rows',
      // XXX
