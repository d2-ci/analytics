...
const DYNAMIC_ORG_UNITS = [_index.USER_ORG_UNIT, _index.USER_ORG_UNIT_CHILDREN, _index.USER_ORG_UNIT_GRANDCHILDREN];
const OrgUnitDimension = _ref => {
  ...
  const onLevelChange = ids => {
    const items = ids.map(id => ({
      id: _index.ouIdHelper.addLevelPrefix(id),
      name: ouLevels.find(level => level.id === id).displayName
    }));
    onSelect({
      dimensionId: _predefinedDimensions.DIMENSION_ID_ORGUNIT,
      items: [...selected.filter(ou => !_index.ouIdHelper.hasLevelPrefix(ou.id)), ...items]
    });
  };
  const onGroupChange = ids => {
    const items = ids.map(id => ({
      id: _index.ouIdHelper.addGroupPrefix(id),
      name: ouGroups.find(group => group.id === id).displayName
    }));
    onSelect({
      dimensionId: _predefinedDimensions.DIMENSION_ID_ORGUNIT,
      items: [...selected.filter(ou => !_index.ouIdHelper.hasGroupPrefix(ou.id)), ...items]
    });
  };
  const getSummary = () => {
    let summary;
    if (selected.length) {
      const numberOfOrgUnits = selected.filter(item => !DYNAMIC_ORG_UNITS.includes(item.id) && !_index.ouIdHelper.hasLevelPrefix(item.id) && !_index.ouIdHelper.hasGroupPrefix(item.id)).length;
      const numberOfLevels = selected.filter(item => _index.ouIdHelper.hasLevelPrefix(item.id)).length;
      const numberOfGroups = selected.filter(item => _index.ouIdHelper.hasGroupPrefix(item.id)).length;
      const userOrgUnits = selected.filter(item => DYNAMIC_ORG_UNITS.includes(item.id));
      const parts = [];
      if (numberOfOrgUnits) {
        parts.push(_d2I18n.default.t('{{count}} org units', {
      ...
      if (numberOfLevels) {
        parts.push(_d2I18n.default.t('{{count}} levels', {
      ...
      if (numberOfGroups) {
        parts.push(_d2I18n.default.t('{{count}} groups', {
      ...
      summary = _d2I18n.default.t('Selected: {{commaSeparatedListOfOrganisationUnits}}', {
        keySeparator: '>',
        nsSeparator: '|',
        commaSeparatedListOfOrganisationUnits: (0, _list.formatList)(parts)
      });
    } else {
      summary = _d2I18n.default.t('Nothing selected');
    }
    return summary;
  };
  return /*#__PURE__*/_react.default.createElement("div", {
    ...
    label: _d2I18n.default.t('User organisation unit'),
    checked: selected.some(item => item.id === _index.USER_ORG_UNIT),
    onChange: _ref2 => {
      let {
        checked
      } = _ref2;
      return onSelectItems({
        id: _index.USER_ORG_UNIT,
        checked,
        displayName: _d2I18n.default.t('User organisation unit')
      });
    },
    dense: true
  }), /*#__PURE__*/_react.default.createElement(_ui.Checkbox, {
    label: _d2I18n.default.t('User sub-units'),
    checked: selected.some(item => item.id === _index.USER_ORG_UNIT_CHILDREN),
    onChange: _ref3 => {
      let {
        checked
      } = _ref3;
      return onSelectItems({
        id: _index.USER_ORG_UNIT_CHILDREN,
        checked,
        displayName: _d2I18n.default.t('User sub-units')
      });
    },
    dense: true
  }), /*#__PURE__*/_react.default.createElement(_ui.Checkbox, {
    label: _d2I18n.default.t('User sub-x2-units'),
    checked: selected.some(item => item.id === _index.USER_ORG_UNIT_GRANDCHILDREN),
    onChange: _ref4 => {
      let {
        checked
      } = _ref4;
      return onSelectItems({
        id: _index.USER_ORG_UNIT_GRANDCHILDREN,
        checked,
        displayName: _d2I18n.default.t('User sub-x2-units')
      });
    },
    dense: true
    ...
    roots: roots,
    initiallyExpanded: [...(roots.length === 1 ? [`/${roots[0]}`] : []), ...selected.filter(item => !DYNAMIC_ORG_UNITS.includes(item.id) && !_index.ouIdHelper.hasLevelPrefix(item.id) && !_index.ouIdHelper.hasGroupPrefix(item.id)).map(item => item.path.substring(0, item.path.lastIndexOf('/'))).filter(path => path)],
    selected: selected.filter(item => !DYNAMIC_ORG_UNITS.includes(item.id) && !_index.ouIdHelper.hasLevelPrefix(item.id) && !_index.ouIdHelper.hasGroupPrefix(item.id)).map(item => item.path),
    ...
    selected: ouLevels.length ? selected.filter(item => _index.ouIdHelper.hasLevelPrefix(item.id)).map(item => _index.ouIdHelper.removePrefix(item.id)) : [],
    onChange: _ref5 => {
      let {
        selected
      } = _ref5;
    ...
    placeholder: _d2I18n.default.t('Select a level'),
    ...
    selected: ouGroups.length ? selected.filter(item => _index.ouIdHelper.hasGroupPrefix(item.id)).map(item => _index.ouIdHelper.removePrefix(item.id)) : [],
    onChange: _ref6 => {
      let {
        selected
      } = _ref6;
    ...
    placeholder: _d2I18n.default.t('Select a group'),
  ...
  }, _d2I18n.default.t('Deselect all')))), /*#__PURE__*/_react.default.createElement(_style.default, {
