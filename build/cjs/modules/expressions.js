...
const getOperators = () => [{
  ...
  value: '',
  label: _d2I18n.default.t('Number'),
...
const validateExpression = expression => {
  ...
  if (!expression) {
    ...
    result = {
      ...
      message: _d2I18n.default.t('Formula is empty. Add items to the formula from the lists on the left.')
    };
    ...
    result = {
      status: INVALID_EXPRESSION,
      message: _d2I18n.default.t('Consecutive data elements')
    };
  } else if (/^[+\-*/]|[+\-*/]$/.test(expression)) {
    // starting or ending with a math operator
    result = {
      status: INVALID_EXPRESSION,
      message: _d2I18n.default.t('Starts or ends with a math operator')
    };
  } else if (/\(\)/.test(expression)) {
    // contains an empty set of parentheses
    result = {
      status: INVALID_EXPRESSION,
      message: _d2I18n.default.t('Empty parentheses')
    };
  } else if (leftParenthesisCount > rightParenthesisCount) {
    // ( but no )
    result = {
      status: INVALID_EXPRESSION,
      message: _d2I18n.default.t('Missing right parenthesis )')
    };
  } else if (rightParenthesisCount > leftParenthesisCount) {
    // ) but no (
    result = {
      status: INVALID_EXPRESSION,
      message: _d2I18n.default.t('Missing left parenthesis (')
    };
  }
  return result;
};
