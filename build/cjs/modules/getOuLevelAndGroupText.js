...
const getOuLevelAndGroupText = (filter, metaData) => {
  ...
  const hasOuLevel = items.some(item => _index.ouIdHelper.hasLevelPrefix(item.id));
  const hasOuGroup = items.some(item => _index.ouIdHelper.hasGroupPrefix(item.id));
...
const getLevelAndGroupText = (items, metaData, isLevel) => {
  ...
  const dynamicOuItems = items.filter(item => isLevel ? _index.ouIdHelper.hasLevelPrefix(item.id) : _index.ouIdHelper.hasGroupPrefix(item.id));
  const lastItem = dynamicOuItems.length > 1 ? dynamicOuItems.pop() : null;
  const dynamicOuNames = dynamicOuItems.map(item => getNameFromMetadata(_index.ouIdHelper.removePrefix(item.id))).join(', ');
  let allDynamicOuNames;
  if (lastItem) {
    const lastOuName = getNameFromMetadata(_index.ouIdHelper.removePrefix(lastItem.id));
  ...
  const staticOuNames = items.filter(item => !_index.ouIdHelper.hasGroupPrefix(item.id) && !_index.ouIdHelper.hasLevelPrefix(item.id)).map(item => getNameFromMetadata(item.id)).join(', ');
