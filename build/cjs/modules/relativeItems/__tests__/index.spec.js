...
describe('relativeItems', () => {
  describe('hasRelativeItems', () => {
    it('returns false for no input', () => {
      expect((0, _index.hasRelativeItems)()).toBe(false);
    });
    it('returns false for no dimension and empty array', () => {
      expect((0, _index.hasRelativeItems)('', [])).toBe(false);
    });
    it('returns true for Assigned Categories and no input', () => {
      const dimension = _predefinedDimensions.DIMENSION_ID_ASSIGNED_CATEGORIES;
      expect((0, _index.hasRelativeItems)(dimension)).toBe(true);
    });
    it('returns true for Assigned Categories and empty array', () => {
      const dimension = _predefinedDimensions.DIMENSION_ID_ASSIGNED_CATEGORIES;
      const itemIds = [];
      expect((0, _index.hasRelativeItems)(dimension, itemIds)).toBe(true);
    });
    it('returns false for period and no input', () => {
      const dimension = _predefinedDimensions.DIMENSION_ID_PERIOD;
      expect((0, _index.hasRelativeItems)(dimension)).toBe(false);
    });
    it('returns false for period and no item ids', () => {
      const dimension = _predefinedDimensions.DIMENSION_ID_PERIOD;
      const itemIds = [];
      expect((0, _index.hasRelativeItems)(dimension, itemIds)).toBe(false);
    });
    ...
    it('returns false for org unit and no input', () => {
      const dimension = _predefinedDimensions.DIMENSION_ID_ORGUNIT;
      expect((0, _index.hasRelativeItems)(dimension)).toBe(false);
    });
