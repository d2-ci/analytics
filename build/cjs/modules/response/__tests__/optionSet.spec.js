...
const headerIndex = 0;
const testId = _optionsetDataHidena.default.headers[headerIndex].name;
...
describe('optionSet', () => {
  ...
  describe('getOptionIdRows', () => {
    it('should replace option codes with ids in the right index', () => {
      expect((0, _optionSet.getOptionIdRows)(_optionsetDataOrg.default.rows, testOptionCodeIdMap, headerIndex)).toEqual(_optionsetDataHidena.default.rows);
    });
  });
  describe('applyOptionSetHandler', () => {
    it('should return the transformed response', () => {
      expect((0, _optionSet.applyOptionSetHandler)(_optionsetDataOrg.default, headerIndex)).toEqual(_optionsetDataHidena.default);
    });
  });
});