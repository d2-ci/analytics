...
const isRegressionIneligible = type => (0, _arrayContains.default)([_visTypes.VIS_TYPE_GAUGE, _visTypes.VIS_TYPE_PIE], type);

exports.isRegressionIneligible = isRegressionIneligible;

function _default(layout, series, isStacked) {
...
function getDefaultTrendLines(layout, series, isStacked) {
  ...
  if (isStacked) {
    newSeries.push(...series, Object.assign({}, getRegressionObj((0, _getStackedData.default)(series, layout), layout.regressionType)));
  } else {
    series.forEach(seriesObj => {
      newSeries.push(seriesObj, Object.assign({}, getRegressionObj(seriesObj.data, layout.regressionType), {
        name: seriesObj.name ? _index.default.t('{{seriesName}} (trend)', {
          seriesName: seriesObj.name
        }) : _index.default.t('Trend'),
...
function getTwoCategoryTrendlineConfig(groupObj, groupIndex, groupRegressionObject, regressionType, trendlineId) {
  const trendlineConfig = getRegressionObj(groupObj, regressionType);
  groupRegressionObject.splice(groupIndex * groupObj.length, groupObj.length, ...trendlineConfig.data.map(point => point[1]));
  trendlineConfig.data = groupRegressionObject; // link trendlines
...
function getTwoCategoryTrendLines(layout, series, isStacked) {
  ...
  if (isStacked) {
    ...
    stackedData.forEach((groupObj, groupIndex) => {
      const trendlineConfig = getTwoCategoryTrendlineConfig(groupObj, groupIndex, groupRegressionTemplate.slice(), layout.regressionType, 'trendline-stacked');
    ...
    series.forEach(seriesObj => {
      ...
      if (!seriesObj.custom.isTwoCategoryFakeSerie) {
        ...
        seriesObj.custom.data.forEach((groupObj, groupIndex) => {
          const trendlineConfig = getTwoCategoryTrendlineConfig(groupObj, groupIndex, groupRegressionTemplate.slice(), layout.regressionType, trendlineSerieId);
          newSeries.push(Object.assign({}, trendlineConfig, {
            name: seriesObj.name ? _index.default.t('{{seriesName}} (trend)', {
              seriesName: seriesObj.name
            }) : _index.default.t('Trend'),
            color: getDarkerColor(seriesObj.color)
...
function getDarkerColor(color) {
  ...
  if (Object.prototype.hasOwnProperty.call(color, 'patternIndex')) {
    const colorSetPatterns = _colorSets.colorSets[_colorSets.COLOR_SET_PATTERNS].patterns;
    color = colorSetPatterns[color.patternIndex].color || DEFAULT_TRENDLINE.color;
  }
...
function getRegressionObj(data, regressionType) {
  ...
  const regressionData = data.some(i => Array.isArray(i)) ? data : getRegressionData(data);
