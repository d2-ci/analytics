...
function _default(series, store) {
  ...
  const data = [...new Set(storeRows.map(row => row[1]))].map(row => {
    const y = Number((storeRows.find(item => item.includes(row) && item.includes(series[0].id)) || [])[2]);
    const x = Number((storeRows.find(item => item.includes(row) && item.includes(series[1].id)) || [])[2]);
