...
function getFirstLastValueIndexes(series) {
  ...
  series.forEach(seriesObj => {
    ...
    i = data.reverse().findIndex(value => value !== undefined && value !== null);
...
function _default(config, layout) {
  ...
  series.forEach(seriesObj => {
    ...
    groups.forEach((groupObj, groupIndex) => {
      if (groupObj.some(value => value !== undefined && value !== null)) {
        firstGroupWithValuesIndex = firstGroupWithValuesIndex !== undefined ? Math.min(firstGroupWithValuesIndex, groupIndex) : groupIndex;
      }
    });
    groups.reverse().forEach((groupObj, groupIndex) => {
      if (groupObj.some(value => value !== undefined && value !== null)) {
        lastGroupWithValuesIndex = Math.max(lastGroupWithValuesIndex, groups.length - 1 - groupIndex);
      }
    });
  });
  return {
...
function getTwoCategoryTrimmedConfig(config, layout) {
  const filteredSeries = config.series.filter(serieObj => !serieObj.custom.isTwoCategoryFakeSerie);
  ...
  if (emptyGroupIndexes.length && config.xAxis && config.series) {
    ...
    const trimmedSeries = config.series.map(seriesObj => {
      if (seriesObj.custom.isTwoCategoryFakeSerie) {
        seriesObj.data = cleanData(seriesObj.data, emptyGroupIndexes, firstGroupWithValuesIndex, lastGroupWithValuesIndex, layout.hideEmptyRowItems);
      } else {
        seriesObj.custom.data = cleanData(seriesObj.custom.data, emptyGroupIndexes, firstGroupWithValuesIndex, lastGroupWithValuesIndex, layout.hideEmptyRowItems);
    ...
    const trimmedXAxis = config.xAxis.map(xAxis => {
      xAxis.categories = cleanData(xAxis.categories, emptyGroupIndexes, firstGroupWithValuesIndex, lastGroupWithValuesIndex, layout.hideEmptyRowItems);
      return xAxis;
    });
...
function getDefaultTrimmedConfig(config, layout) {
  ...
  if (emptySeriesIndexes.length && config.xAxis && config.series) {
    ...
    const trimmedSeries = config.series.map(seriesObj => ({ ...seriesObj,
      data: cleanData(seriesObj.data, emptySeriesIndexes, firstValueIndex, lastValueIndex, layout.hideEmptyRowItems)
    }));
    const trimmedXAxis = [{ ...config.xAxis,
      categories: cleanData(config.xAxis[0].categories, emptySeriesIndexes, firstValueIndex, lastValueIndex, layout.hideEmptyRowItems)
    }];
    return Object.assign({}, config, {
      series: trimmedSeries,
      xAxis: trimmedXAxis
