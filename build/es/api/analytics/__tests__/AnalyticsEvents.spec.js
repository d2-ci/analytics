...
describe('analytics.events', () => {
  ...
  describe('.getCount()', () => {
    beforeEach(() => {
      events = new AnalyticsEvents(new DataEngineMock());
      request = new AnalyticsRequest().withProgram('eBAyeGv0exc').addPeriodDimension('LAST_YEAR').addOrgUnitDimension('ImspTQPwCqd').addDimension('qrur9Dvnyt5:LT:50');
  ...
  describe('.getQuery()', () => {
    beforeEach(() => {
      events = new AnalyticsEvents(new DataEngineMock());
      request = new AnalyticsRequest().addOrgUnitDimension('ImspTQPwCqd').addDimension('qrur9Dvnyt5:LT:50').addPeriodFilter('LAST_MONTH').withStage('Zj7UnCAulEk').withPage(1).withPageSize(5);
