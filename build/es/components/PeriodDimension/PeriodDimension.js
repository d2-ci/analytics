...
const PeriodDimension = ({
  ...
  const enabledPeriodTypesResult = useDataQuery(supportsEnabledPeriodTypes ? enabledPeriodTypesQuery : {
    skip: true
  });

  // Conditionally fetch financial year start setting for v43+
  const financialYearStartResult = useDataQuery(supportsEnabledPeriodTypes ? financialYearStartQuery : {
    skip: true
  });

  // Conditionally fetch analysis relative period setting for v43+
  const analysisRelativePeriodResult = useDataQuery(supportsEnabledPeriodTypes ? analysisRelativePeriodQuery : {
    skip: true
  });
  const {
    calendar = 'gregory'
