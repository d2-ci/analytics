...
import { VIS_TYPE_COLUMN, VIS_TYPE_YEAR_OVER_YEAR_LINE, VIS_TYPE_BAR, VIS_TYPE_PIE, VIS_TYPE_DONUT, VIS_TYPE_SINGLE_VALUE, VIS_TYPE_OUTLIER_TABLE } from '../visTypes.js';
const someId = 'someId';
...
describe('getAdaptedUiLayoutByType', () => {
  it('column: moves all extra dimensions in columns and rows to filters', () => {
    const initialState = {
      ...
      [AXIS_ID_ROWS]: [],
      [AXIS_ID_FILTERS]: [DIMENSION_ID_PERIOD, DIMENSION_ID_ORGUNIT, otherId]
    };
    const actualState = getAdaptedUiLayoutByType(initialState, VIS_TYPE_COLUMN);
    const expectedState = {
      [AXIS_ID_COLUMNS]: [DIMENSION_ID_DATA],
      [AXIS_ID_ROWS]: [],
      [AXIS_ID_FILTERS]: [DIMENSION_ID_PERIOD, DIMENSION_ID_ORGUNIT, otherId, someId]
    };
  ...
  it('pie: moves all column and row dimensions to filter except the first column dimension', () => {
    ...
    const expectedState = {
      [AXIS_ID_COLUMNS]: [DIMENSION_ID_DATA],
      [AXIS_ID_ROWS]: [],
      [AXIS_ID_FILTERS]: [DIMENSION_ID_ORGUNIT, someId, DIMENSION_ID_PERIOD, otherId]
    };
    expect(actualState).toEqual(expectedState);
  });
  it('pie: moves the first row dimension to series and the rest to filter', () => {
    ...
    const expectedState = {
      [AXIS_ID_COLUMNS]: [DIMENSION_ID_DATA],
      [AXIS_ID_ROWS]: [],
      [AXIS_ID_FILTERS]: [DIMENSION_ID_ORGUNIT, DIMENSION_ID_PERIOD, someId]
    };
    expect(actualState).toEqual(expectedState);
  });
  it('yoy: removes the "pe" dimension and moves all other dimensions to filter with dimension strings', () => {
    ...
    const actualState = getAdaptedUiLayoutByType(initialState, VIS_TYPE_YEAR_OVER_YEAR_LINE);
    const expectedState = {
      [AXIS_ID_COLUMNS]: [],
      [AXIS_ID_ROWS]: [],
      [AXIS_ID_FILTERS]: [DIMENSION_ID_ORGUNIT, DIMENSION_ID_DATA, someId, otherId]
    };
    expect(actualState).toEqual(expectedState);
  });
  ...
  it('pivot -> sv with dimension strings', () => {
    ...
    const actualLayout = getAdaptedUiLayoutByType(initialLayout, VIS_TYPE_SINGLE_VALUE);
    const expectedLayout = {
      [AXIS_ID_COLUMNS]: [DIMENSION_ID_DATA],
      [AXIS_ID_ROWS]: [],
      [AXIS_ID_FILTERS]: [someId, DIMENSION_ID_PERIOD, DIMENSION_ID_ORGUNIT]
    };
