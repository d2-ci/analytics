...
import { VIS_TYPE_YEAR_OVER_YEAR_LINE, VIS_TYPE_YEAR_OVER_YEAR_COLUMN, VIS_TYPE_PIE, VIS_TYPE_DONUT, VIS_TYPE_GAUGE, VIS_TYPE_SINGLE_VALUE, VIS_TYPE_PIVOT_TABLE, VIS_TYPE_SCATTER, VIS_TYPE_OUTLIER_TABLE, isTwoCategoryChartType } from './visTypes.js';
...
const getDualCategoryLayout = layout => {
  ...
  return {
    [AXIS_ID_COLUMNS]: columns.length ? [columns.shift()] : columns,
    [AXIS_ID_ROWS]: rows.length > 2 ? rows.splice(0, 2) : rows.splice(0, rows.length),
    [AXIS_ID_FILTERS]: [...layout[AXIS_ID_FILTERS], ...columns, ...rows]
  };
};
...
const getYearOverYearLayout = layout => ({
  [AXIS_ID_COLUMNS]: [],
  [AXIS_ID_ROWS]: [],
  [AXIS_ID_FILTERS]: [...layout[AXIS_ID_FILTERS], ...layout[AXIS_ID_COLUMNS], ...layout[AXIS_ID_ROWS]].filter(dim => getDimensionId(dim) !== DIMENSION_ID_PERIOD)
});
const getScatterLayout = layout => ({
  [AXIS_ID_COLUMNS]: [DIMENSION_ID_DATA],
  [AXIS_ID_ROWS]: [DIMENSION_ID_ORGUNIT],
  [AXIS_ID_FILTERS]: [...layout[AXIS_ID_COLUMNS], ...layout[AXIS_ID_ROWS], ...layout[AXIS_ID_FILTERS]].filter(dim => ![DIMENSION_ID_DATA, DIMENSION_ID_ORGUNIT].includes(getDimensionId(dim)))
});
