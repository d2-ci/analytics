...
describe('layoutGetDimensionIdItemIdsObject', () => {
  it('should return a dimensionId:[itemdIds] object based on the layout', () => {
    ...
    TEST_DIMENSIONS_IN_LAYOUT.forEach(dimension => {
      expectedState[dimension[DIMENSION_PROP_ID.name]] = dimension[DIMENSION_PROP_ITEMS.name].map(item => item[ITEM_PROP_ID.name]);
    });
    expect(layoutGetDimensionIdItemIdsObject(TEST_LAYOUT)).toEqual(expectedState);
  });
});