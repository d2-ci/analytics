...
import { testResourceRules, testResourceRequiredProps, testResourceAllRuleProps } from '../rules.js'; // Consts

const lockableDims = [DIMENSION_ID_DATA, DIMENSION_ID_PERIOD, DIMENSION_ID_ORGUNIT];
const disallowableDims = [DIMENSION_ID_DATA, DIMENSION_ID_PERIOD]; // Helper fns
...
const onlyRulesWithProp = ruleProp => testResourceRules.filter(rule => rule[ruleProp]); // Partial tests
...
describe('verify all rules', () => {
  it('implements all required props', () => {
    expect(testResourceRules.every(rule => testResourceRequiredProps.every(val => Object.keys(rule).indexOf(val) !== -1))).toBe(true);
  });
  it('only implements allowed rule props', () => {
    expect(testResourceRules.every(rule => allArrayItemsAreValid(Object.keys(rule), Object.values(testResourceAllRuleProps)))).toBe(true);
  });
});
