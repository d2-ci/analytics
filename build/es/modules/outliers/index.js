...
export const getOutlierHelper = function (data) {
  ...
  if (config[PROP_THRESHOLD_FACTOR] <= 0 && [STANDARD_Z_SCORE, MODIFIED_Z_SCORE].includes(config[PROP_OUTLIER_METHOD])) {
    return null;
  }
  const normalizationHelper = getNormalizationHelper(data, config.normalizationMethod);
  const options = {
  ...
  if (config[PROP_EXTREME_LINES][PROP_ENABLED] && isNumeric(config[PROP_EXTREME_LINES][PROP_EXTREME_LINES_VALUE])) {
