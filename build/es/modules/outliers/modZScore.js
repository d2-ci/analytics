...
export const getMean = values => values.reduce((total, value) => total + value, 0) / values.length;
...
export const getModZScoreMAD0 = (value, median, meanAD) => (value - median) / (meanAD * MEAN_AD_CORRECTION);
...
export const getDataWithZScore = function (dataWithNormalization) {
  ...
  let dataWithZScore;
  if (medianAD === 0) {
    const meanAD = isNumber(cache.meanAD) ? cache.meanAD : getMeanAbsoluteDeviation(normalizedData);
...
export const getModZScoreHelper = (normalizationHelper, config, _ref) => {
  ...
  const [lowThreshold, highThreshold] = medianAD === 0 ? getModZScoreMAD0Thresholds(config[PROP_THRESHOLD_FACTOR], meanAD, median) : getModZScoreThresholds(config[PROP_THRESHOLD_FACTOR], medianAD, median);
