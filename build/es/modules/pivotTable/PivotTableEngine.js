...
const ORGUNIT_DIMENSION_IDS = [DIMENSION_ID_ORGUNIT, DIMENSION_ID_ENROLLMENT_ORGUNIT];
...
const isOrgUnitDimension = ({
  dimension,
  meta
}) => (meta === null || meta === void 0 ? void 0 : meta.dimensionType) === DIMENSION_TYPE_ORGANISATION_UNIT || ORGUNIT_DIMENSION_IDS.includes(dimension.split('.').pop());
...
export class PivotTableEngine {
  ...
  getDimensionLabel(rowLevel, columnLevel) {
    ...
    if (rowLevel === lastRowLevel && this.dimensionLookup.rows[lastRowLevel] && columnLevel === lastColumnLevel && this.dimensionLookup.columns[lastColumnLevel]) {
      return `${i18n.t(this.dimensionLookup.rows[lastRowLevel].meta.name)} / ${i18n.t(this.dimensionLookup.columns[lastColumnLevel].meta.name)}`;
    }
    if (lastRowLevel === -1) {
