...
import { applyNumericHandler, getNumericDimension, getNumericItems, getNumericRows, getPrefixedValue, getUnique, sortStringsAsNumbersAsc } from '../numeric.js';
...
describe('numeric', () => {
  ...
  describe('getNumericRows', () => {
    it('prefixes value at headerIndex for each row', () => {
      ...
      const headerIndex = 1;
      expect(getNumericRows(rows, headerIndex, dimensionId)).toEqual([['a', 'Zj7UnCAulEk.fWIAEtYVEGk:1', 'x'], ['b', 'Zj7UnCAulEk.fWIAEtYVEGk:2', 'y'], ['c', 'Zj7UnCAulEk.fWIAEtYVEGk:10', 'z']]);
    });
    it('does not modify rows where the value at headerIndex is an empty string', () => {
      const rows = [['a', '', 'x'], ['b', '2', 'y']];
      const headerIndex = 1;
      expect(getNumericRows(rows, headerIndex, dimensionId)).toEqual([['a', '', 'x'], ['b', 'Zj7UnCAulEk.fWIAEtYVEGk:2', 'y']]);
    });
  ...
  describe('applyNumericHandler', () => {
    it('should return the transformed response', () => {
      expect(applyNumericHandler(responseOrg, headerIndex)).toEqual(response);
    });
  });
});