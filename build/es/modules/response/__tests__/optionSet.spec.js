...
describe('optionSet', () => {
  ...
  describe('getOptionIdRows', () => {
    it('should replace option codes with ids in the right index', () => {
      expect(getOptionIdRows(responseOrg.rows, testOptionCodeIdMap, headerIndex)).toEqual(responseHideNa.rows);
    });
  });
  describe('applyOptionSetHandler', () => {
    it('should return the transformed response', () => {
      expect(applyOptionSetHandler(responseOrg, headerIndex)).toEqual(responseHideNa);
    });
  });
});