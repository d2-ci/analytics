...
describe('default', () => {
  ...
  describe('getItems', () => {
    ...
    it('resolves names from the items map when value matches an item id', () => {
      ...
      expect(getItems(['C6nZpLKjEJr', 'CW81uF03hvV'], 'kO3z4Dhc038.LFsZ8v5v7rq', {
        items
      })).toEqual({
    ...
    it('falls back to the raw value when no matching item exists (free text)', () => {
      ...
      expect(getItems(['email@address.com'], 'jfuXZB3A1ko.RUZ2EBP6HQn', {
        items
      })).toEqual({
    ...
    it('prefers the formatter over the items lookup when both are provided', () => {
      ...
      expect(getItems(['1'], 'foo', {
        items,
        itemFormatter: () => 'Yes'
      })).toEqual({
        [`foo${PREFIX_SEPARATOR}1`]: {
          name: 'Yes'
        }
      });
    });
