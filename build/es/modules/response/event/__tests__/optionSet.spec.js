...
import { applyOptionSetHandler, getOptionCodeIdMap, getOptionIdRows } from '../optionSet.js';
...
describe('optionSet', () => {
  ...
  describe('applyOptionSetHandler', () => {
    ...
    it('maps D2__NOVALUE to NO_VALUE and adds the NO_VALUE item', () => {
      ...
      expect(result.metaData.dimensions['stageA.element1']).toEqual(['optionId1', 'optionId2', NO_VALUE]);
      expect(result.metaData.items[NO_VALUE]).toEqual(NO_VALUE_ITEM);
      expect(result.rows).toEqual([['optionId1'], ['optionId2'], [NO_VALUE]]);
    });
  });
});