...
export const transformResponse = (response, {
  ...
  response.headers.forEach((header, index) => {
    if (header.meta) {
      if (header.optionSet) {
        transformedResponse = applyOptionSetHandler(transformedResponse, index);
      } else if (isNumericValueType(header.valueType) && !header.legendSet) {
        transformedResponse = applyNumericHandler(transformedResponse, index);
      } else if (isBooleanValueType(header.valueType)) {
        transformedResponse = applyBooleanHandler(transformedResponse, index);
      }
    }
  });
  if (hideNaData) {
