...
export function positionElements(valueElement, subTextElement, iconElement, spacing) {
  ...
  // DEBUGGING THE RECT
  const debugRect = this.renderer.rect(layoutRect.x, layoutRect.y, layoutRect.width, layoutRect.height).attr({
    fill: 'orange',
    opacity: 0.3
  }).add();
  ...
  if (subTextElement) {
    ...
    subTextElement.attr({
      x: iconElement ? layoutRect.x + spacing.iconSize + spacing.iconGap : layoutRect.x,
...
function computeLayoutRect(valueElement, subTextElement, iconElement, spacing) {
  ...
  if (subTextElement) {
    ...
    console.log(`What is bigger? valueWidth: ${width} subTexttWidth ${subTextRect.width}`);
