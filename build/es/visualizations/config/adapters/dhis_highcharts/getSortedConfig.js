...
function getTwoCategorySortedConfig(config, layout, stacked) {
  ...
  const indexOrder = []; // loop through serie groups
  ...
  sortedConfig.series = series.map((seriesObj, seriesIndex) => {
    if (seriesObj.custom.isTwoCategoryFakeSerie) {
      ...
      seriesObj.custom.data = seriesObj.custom.data.map((groupObj, groupIndex) => {
        ...
        if (seriesIndex === 0) {
          indexOrder[groupIndex] = getIndexOrder(stacked ? stackedData[groupIndex] : groupObj, layout); // reorder 2nd category labels only once
...
function getDefaultSortedConfig(config, layout, stacked) {
  ...
  const series = config.series;
  const indexOrder = getIndexOrder(stacked ? getStackedData(series, layout) : series[0].data, layout);
  const sortedConfig = Object.assign({}, config);
  ...
  sortedConfig.series = series.map(seriesObj => ({ ...seriesObj,
