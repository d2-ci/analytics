...
import { VIS_TYPE_PIE, VIS_TYPE_DONUT, VIS_TYPE_GAUGE, isDualAxisType, isYearOverYear, VIS_TYPE_LINE, VIS_TYPE_SCATTER, VIS_TYPE_SINGLE_VALUE } from '../../../../../modules/visTypes.js';
...
function getIndexColorPatternMap(series, layout, extraOptions) {
  ...
  const colors = ((_colorSets$layout$col = colorSets[layout.colorSet]) === null || _colorSets$layout$col === void 0 ? void 0 : _colorSets$layout$col.colors) || extraOptions.colors;
  return series.reduce((map, s, index) => {
    map[index] = layout.colorSet === COLOR_SET_PATTERNS ? {
...
function getIdColorMap(series, layout, extraOptions) {
  ...
  if (isDualAxisType(layout.type) && hasCustomAxes(filteredSeries) && !axisHasRelativeItems(layout.columns)) {
    ...
    const colorsByAxis = Object.keys(axisIdsMap).reduce((map, axis) => {
      ...
      map[axis] = numberOfIds > 1 ? generateColors(theme[axis].startColor, theme[axis].endColor, numberOfIds) : [theme[axis].mainColor];
      return map;
    }, {});
...
function getDefault(_ref) {
  ...
  series.forEach((seriesObj, index) => {
    ...
    // axis number
    seriesObj.yAxis = isDualAxisType(layout.type) && !axisHasRelativeItems(layout.columns) ? getAxisStringFromId(fullIdAxisMap[seriesObj.id]) : getAxisStringFromId(0);
...
export default function (_ref2) {
  ...
  switch (layout.type) {
    ...
    case VIS_TYPE_PIE:
      ...
      break;
    case VIS_TYPE_DONUT:
      series = getDonut(series, Object.values(getIdColorMap(series, layout, extraOptions)));
      break;
    case VIS_TYPE_GAUGE:
